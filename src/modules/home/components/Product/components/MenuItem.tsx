...
interface OrderItem {
  ...
  const MenuItem: React.FC<MenuItemProps> = ({ item, onAddToCart, onEdit,currentPage,orders }) => {
  ...
  order.orderItems.some(itemOrdered => itemOrdered.productId === item.id)
);
    return (
      <Card className="flex flex-col h-full overflow-hidden hover:shadow-lg transition-shadow" onClick={handleAddToCart}>
