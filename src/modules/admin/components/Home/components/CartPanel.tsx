...
export default function CartPanel({
  ...
  return (
    <div
      className={`fixed top-0 right-0 h-screen w-80 bg-white shadow-lg transition-transform duration-300 ${
        isCartOpen ? 'translate-x-0' : 'translate-x-full'
      }`}
    >
      <div className='p-4 h-full flex flex-col'>
        <div className='flex justify-between items-center mb-4'>
          <h3 className='text-lg font-semibold'>Đơn hàng #{Math.floor(Math.random() * 1000000)}</h3>
          <Button
            variant='ghost'
            size='icon'
            onClick={closeCart}
            ...
            {cart.map((item) => (
              ...
              >
                ...
                <div className='flex-1'>
                  ...
                  <p className='text-gray-600 text-sm'>{item.price.toLocaleString('vi-VN')} đ</p>
